...
export function SuspectedPlayers() {
	...
	const players = data ? flatData(data).filter(player => player.violations.length > 0) : []
		...
		<div className="suspect-players-wrapper">
			<div className="suspect-players-content">
				...
				<div className="suspect-players-list">
					{players.map(player => {
						const violationMessages = player.violations.map(violationGroup => {

							if (violationGroup.name === 'unreliableSL') {
								const SLViolationMessages = violationGroup.payload.map(violation => {
									...
									if (violation.violation === 'SQ drop') {
										return `Бросил отряд ${player.squadID !== violation.squadID ? `${violation.squadID}` : ''} ${player.teamId !== violation.teamID ? 'в противоположной команде' : ''}`
									}
						...
						return <div key={player.id} className="suspect-players-item" onClick={() => dispatch(showModal({player}))}>
							<RiAlarmWarningLine className="suspect-players-icon" style={{color: '#fdac34'}}/>
							<div>
