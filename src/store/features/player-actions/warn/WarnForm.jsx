...
export function WarnForm(props) {
	...
	const options = warnReasonText.map(i => {
		return <option key={i.id} value={i.text}>{i.text}</option>
	})
	...
	return <div>
		<div>
			<p>Выберите сообщение: <span>(можно вписать)</span></p>
			<select className="select-options" value={warnReason} onChange={(e) => setWarnReason(e.target.value)}>
				{options}
			</select>
			<p>Введите сообщение:</p>
			<input className="select-options" type="text" value={warnReason} onChange={(e) => setWarnReason(e.target.value)}/>
		</div>
		<button className="action-btn" onClick={warnPlayerHandler}>ОТПРАВИТЬ СООБЩЕНИЕ</button>
	</div>
}
