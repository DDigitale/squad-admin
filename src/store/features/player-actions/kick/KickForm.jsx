...
export function KickForm(props) {
	...
	return <div>
		<div>
			<p>Причина кика:</p>
			<select className="select-options" value={kickReason} onChange={(e) => setKickReason(e.target.value)}>
				{options}
			</select>
		</div>
		<button className="action-btn" onClick={kickPlayerHandler}>КИКНУТЬ</button>
	</div>
}
